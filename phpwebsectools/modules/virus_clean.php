...
class phpwebsectools_modules_virus_clean {
    ...
    public function load_static_definitions() {
            ...
            $dir = dirname(__FILE__).DIRECTORY_SEPARATOR.'virus_clean'.DIRECTORY_SEPARATOR.'definitions';
    ...
    function isPHP($inode) {
       if (is_dir($inode)) return false;       
	     $a = explode(".",$inode);
	     $ext = (count($a) > 1) ? array_pop($a) : null;
	     return "php" == strtolower($ext); 
    }
         ...
         if ($this->isPHP($inode)) {
             ...
             if ($modified) {
                 ...
                 if (false !== file_put_contents($inode,$php_file_content)) {
                     phpwebsectools::log("Cleaning virus infection in $inode [success]");
                     @unlink($inode.'.infected.'.$t);
                     echo "<li class=\"cleaned\">$inode [cleaned]</li>";
                 } else {
                     phpwebsectools::log("Cleaning virus infection in $inode [fail]");
    ...
    public function clean($inode) {
     ...
     public static function install() {
         ...
         $phpwebsectools_clean_modules_classes["virus_clean"] = "phpwebsectools_modules_virus_clean";    
     }
    ...
    function __construct($cleaner,$silent = false) {
         ...
         if (!$silent) {                  
          ?>
          <p>PHP Virus cleaner initialized...</p>
          <?php
          }
...
phpwebsectools_modules_virus_clean::install();